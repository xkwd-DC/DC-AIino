...
>
> 最近更新：**2026-05-27 中午**（🌞 Wave 6 收尾 — docs/11 v2 升级 + README 国家级介绍;一晚累计 23 commits;Issue OPEN 仅剩 3 外部 dependency)

---

## 🚨 关键路线决策

### 🌞 2026-05-27 中午 · Wave 6 收尾

- `3739395` **docs/11 数据集说明 v1 → v2 升级**(341 → 520 行):加 v4 plan 引用 + 鲁棒性三协议 + 字段单位 audit(Issue #23 红旗)+ 演进路径 timeline + 红线对照表
- `100caf2` **README.md 国家级升级介绍**(59 → 137 行):项目元信息 + 4 人开发/5 人法定 + 邱+徐 + 三协议鲁棒性表 + M01-M04 + 三主线 + 文档索引
...
- **#42 计划 doc** by `4b3d46e` — `docs/coord/2026-05-26_混合效应模型_PoC_计划.md` (244 行)。给熊鑫照做,1-2 天工作量,ICC 预期 ≥ 0.85(若成立,直接定量证明 R²=0.907 大部分由省份固定效应贡献)
- **#43 ✅ closed** by `a774d3e` — `docs/09 §6` 加 2 篇 2024 顶刊文献支撑 ρ=-0.055 "互补"叙事:
  - Krishna et al. *TMLR* 2024 — 84% practitioners 见过 XAI 分歧,ρ -0.2~+0.4 是常态;ρ=-0.055 落在常见区间
  - Bilodeau et al. *PNAS* 2024 — SHAP 类方法理论上可证明不优于随机,给"单一方法不可靠"严格数学证明
- **T8 v4 数据扩展计划** by `72d2279`(用户 session 干)— `docs/16_v4_plan.md` (284 行)。8a 致灾 + 8b 耕地像元 + 8c 多年 CLCD,6/01-8/31 三个月窗口
- **v6 基线化收尾** by `236b32f` — `docs/03` 加 banner(已提交版快照, 不动)+ `_craic/04` header 从"草稿"升基线 + `docs/13` 国家级执行规划 v1 入仓
- **下一步 Wave 2**:#26 GCP 安全头 + rate-limit(代码层, 不依赖实例) + #25 a11y WCAG 2.2 AA(4 view 代码层修)
